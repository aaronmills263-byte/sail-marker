...
export default async function DestinationsPage({ searchParams }: PageProps) {
  ...
  const seasonFilter = typeof params.season === "string" ? params.season : "";
  const budgetFilter = typeof params.budget === "string" ? params.budget : "";
  ...
  if (budgetFilter) {
    query = query.eq("price_tier", budgetFilter);
  }

  const { data: destinations } = await query;

  let filtered: CharterDestination[] = (destinations as CharterDestination[]) || [];
